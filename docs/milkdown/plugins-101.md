...
1. *Prepare*: this part will be executed when plugin is registered in milkdown by `.use` method.
2. *Run*: this part will be executed when plugin is actually loaded.
3. *Post*: this part will be executed when plugin is removed by `.remove` method or editor is destroyed.
