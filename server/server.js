...
const requireMainAdmin = async (req, res, next) => {
  try {
    ...
    if (!adminRecord) {
      return res
        .status(403)
        .json({ error: "Access denied. Admin record not found." });
    }
...
app.put("/api/admin/settings/:key", requireMainAdmin, async (req, res) => {
  try {
    ...
    else if (key.includes("EMAIL") || key.includes("PASSWORD"))
      category = "email";
    else if (key.includes("LOGO") || key.includes("COMPANY"))
      category = "branding";
    else if (key.includes("GOOGLE") || key.includes("FACEBOOK"))
      category = "social";
